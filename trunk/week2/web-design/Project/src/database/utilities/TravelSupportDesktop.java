...
//                + "and trf.begin_date>:begin_date and trf.end_date<:end_date";
        ...
        return (List<Trf>) s.createSQLQuery(prepared_statement).addEntity(Trf.class).setInteger("id", id).setDate("begin_date", begin_date).setDate("end_date", end_date).setInteger("fromi", from).setInteger("toi", to).list();
    }
    ...
    //
    public static List<Trf> TrfSameCountryFilterByDateDepartment(Integer id, Date begin_date, Date end_date, String department, Integer page, Integer num) {
        Session s = HibernateUtil.getSession();
        Integer from = page * num;
        Integer to = (page + 1) * num;
...
//                + "and trf.begin_date>:begin_date and trf.end_date<:end_date "
//                + "and department.dep_name=:department";
        return (List<Trf>) s.createSQLQuery(prepared_statement).addEntity(Trf.class).setString("department", department).setInteger("id", id).setDate("begin_date", begin_date).setInteger("fromi", from).setInteger("toi", to).setDate("end_date", end_date).list();
    }
}
