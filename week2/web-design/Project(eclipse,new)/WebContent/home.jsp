...
<!-- Le styles -->
<link href="<c:out value="${pageContext.request.contextPath}"/>/assets/css/bootstrap.css"
	rel="stylesheet">
<style type="text/css">
body {
	padding-top: 60px;
...
<body>
	...
	<div class="container">
		...
		<h1><fmt:message key="page.home.hello1.unauth"/></h1>
		<p><fmt:message key="page.home.hello2.unauth"/><br>
		<p><fmt:message key="page.home.hello3.unauth"/><br>
		<div class="row">
			<div class="span4">
				<form class="well form-vertical"
					action="<%=request.getContextPath()%>/auth" method="post">
					<input type="text" class="span3"
						placeholder="<fmt:message key="form.login.login"/>"
						name="login"> <br> <input type="password"
						class="span3"
		...
		<%} else { %>
		<h1><fmt:message key="page.home.hello1.auth"/>
			<a href="<%=request.getContextPath()%>/quit"><%=session.getAttribute("name") %></a>
		</h1>
		<p><fmt:message key="page.home.hello2.auth"/><br>
			<%} %>	
	</div>
	<!-- /container -->
